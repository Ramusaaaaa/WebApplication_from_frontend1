...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      const rolesResponse = await axios.get('http://localhost:8081/api/roles');

      roles.value = rolesResponse.data;

      const response = await axios.get(`http://localhost:8081/users/${route.params.id}`);
    ...
    const submit = async () => {
      await axios.put(`http://localhost:8081/api/users/${route.params.id}`, data);
